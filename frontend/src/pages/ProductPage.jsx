...
export default function ProductPage() {
  const { sku } = useParams();                 // route param
  const [prod, setProd] = useState(null);      // <-- use 'prod', not 'p'
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        // fallback: fetch all then find by id/sku
        try {
  ...
  return (
    <div className="page-wrap">
      <nav className="crumbs" style={{marginBottom: 12}}>
        <Link to="/">Home</Link> <span>›</span> <Link to={`/collections/${encodeURIComponent(prod.collection || "All")}`}>{prod.collection || "All"}</Link> <span>›</span> <span>{prod.name}</span>
      </nav>
              ...
              {imgs.map((src, i) => (
                <button
                  ...
                  style={{
                    ...
                    border: i===active ? "2px solid #111" : "1px solid #e6e6e6",
        ...
        <div>
          ...
          {Array.isArray(prod.sizes) && prod.sizes.length > 0 && (
            <>
              ...
              <div style={{display:"flex", gap:8, flexWrap:"wrap"}}>
                {prod.sizes.map(s => (
                  <span key={s} style={{border:"1px solid #e6e6e6", padding:"8px 12px", borderRadius:999}}>{s}</span>
                ))}
              </div>
            </>
          )}
          ...
          {Array.isArray(prod.colors) && prod.colors.length > 0 && (
            <>
              ...
              <div style={{display:"flex", gap:8, flexWrap:"wrap"}}>
                {prod.colors.map(c => (
                  <span key={c} style={{border:"1px solid #e6e6e6", padding:"8px 12px", borderRadius:999}}>{c}</span>
                ))}
              </div>
            </>
          )}
          ...
          <div className="sticky-cta" style={{marginTop:16}}>
            <button className="btn-solid" style={{width:"100%", height:46}} onClick={() => alert("Pre-order coming soon!")}>
              Pre-order
            </button>
          </div>
        </div>
