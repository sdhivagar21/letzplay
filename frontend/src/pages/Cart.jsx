...
export default function Cart() {
  ...
  useEffect(() => {
    api.get('/api/preorders/mine').then(res => setOrders(res.data)).catch(() => setOrders([]));
  }, []);
  ...
  return (
    <div className="page-wrap">
      <div className="page-head">
        <h1>My Pre-Orders</h1>
        <nav className="crumbs"><Link to="/">Home</Link> <span>›</span> <span>Pre-Orders</span></nav>
      </div>
      ...
      <div className="table">
        ...
        {orders.map(o => (
          <div key={o._id} className="tr">
            <div>
              <div className="t-strong">{o.productName}</div>
              <div className="tiny muted">{o.size} • {o.color}</div>
            </div>
            <div>{o.quantity}</div>
            <div>${o.price}</div>
            <div>{new Date(o.preorderDate).toLocaleDateString()}</div>
